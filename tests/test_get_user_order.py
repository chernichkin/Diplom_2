...
class TestGetUserOrder:
    ...
    def test_get_user_order_with_authorisation_two_burger_success(self, create_user_and_delete):
        ...
        requests.post(Endpoints.URL + Endpoints.ORDER, data=Data.ingredients_hash, headers=headers)
        requests.post(Endpoints.URL + Endpoints.ORDER, data=Data.ingredients_hash, headers=headers)
        response = requests.get(Endpoints.URL + Endpoints.ORDER, headers=headers)
        r = response.json()
    ...
    def test_get_user_order_without_authorisation_401(self, create_user_and_delete):
        ...
        headers = {'Authorization': user.json()['accessToken']}
        requests.post(Endpoints.URL + Endpoints.ORDER, data=Data.ingredients_hash, headers=headers)
        response = requests.get(Endpoints.URL + Endpoints.ORDER)
        r = response.json()
